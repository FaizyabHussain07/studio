  ...
  const handlePageFilesChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
        ...
        try {
            ...
            toast({ title: "Success", description: `${files.length} page(s) added. Remember to save your changes.` });
        } catch (error) {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 pt-4 max-h-[70vh] overflow-y-auto pr-4">
        ...
        {isEditing && (
            <div>
                ...
                <FormDescription>Upload one or more images for the book's pages. Pages will be ordered as you see them below.</FormDescription>
